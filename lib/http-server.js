    ...
    if (req.method === 'GET') {
        ...
        if (req.url === '/facts') {
            ...
            req.on('data', data => {
                ...
                facts.push(fact);
                // facts = Buffer.concat(facts).toString();
                console.log('fact is..', facts);
                res.end(fact);
            })
            // req.on('end', () => {
            //     res.end(fact)
        ...
        else res.end('Try making a post to "/facts"');


        /* attempt 3 */
        // const stream = fs.createReadStream('index.html', { endocing: 'utf8' });
        ...
        // let contents = '';
        // // stream.pipe encapsulates the whole below pattern:    
        // stream.on('data', data => {
        //     contents += data
        //     // this ^ is like this -> res.write(data); // ! response is a write stream!
        // });
        // stream.on('end', () => {
        //     res.end(contents)
        //     // this ^ is like this -> res.end();
        // });

        // //  stream.pipe encapsulates the whole below pattern:    
        // stream.on('data', data => {
        //     res.write(data); // ! response is a write stream!
        // });
        ...
        // });
        // req.on('end', function() {
        //     console.log('facts arr is..', facts);
        ...
        /* attempt 1 */
        // if(!pathnameParts[1]) { // === 'POST/'
        //     res.end('Try adding "facts/<some fact>" in the path.');
        // } 
        // else if (pathnameParts[1]) {
        //     res.end('the end of post');
        // }
    }
